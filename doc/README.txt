...
generated by OpenSIPs SIP Proxy/Registrar.
...
by OpenSIPs when used in combination with MediaProxy regardless of the
...
engine for service providers deploying OpenSIPs as SIP Proxy / Registrar in
...
FreeRadius server, Cisco gateways, MediaProxy and OpenSIPs is required to
...
OpenSIPs radius accounting module. The information can be combined with media
information logged by MediaProxy while a separate OpenSIPs module can display
full traces of all SIP packets the enter and exit the SIP Proxy. CDRs can be
filtered and statistics can be built in real-time to summarize service usage
